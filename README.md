...
Deployment path:
- Recommended: Streamlit Cloud (no Docker required).
...
# OPTIONAL: Dockerfile for reproducible container builds (not required for Streamlit Cloud).
# This file is provided for reproducibility only — you said "No docker" for deployment, so do not use this unless you want a container.
...
RUN apt-get update && apt-get install -y build-essential git \
    libsndfile1 ffmpeg tesseract-ocr \
